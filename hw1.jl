...
# ╔═╡ c2349603-187e-4f3b-ad5e-37117b2d9556
create_bar()
...
function matrix_to_vecvec(matrix)
	result = Vector{Vector{Int64}}(undef, 0)
	rows, _ = size(matrix)
	for i in 1:rows
		push!(result, matrix[i, :])
	end
	return result
end
...
function mean_colors(image)
	...
	red_avg = red_sum / length(image)
	green_avg = green_sum / length(image)
	blue_avg = blue_sum / length(image)
...
quantize(0.267), quantize(0.91), quantize(1.15)
...
red_ = RGB(0.8, 0.1, 0.1)

# ╔═╡ 846b1330-ee0b-11ea-3579-7d90fafd7290
md"Can you invert the picture of Philip?"

# ╔═╡ f03b8233-3ba7-4ab9-b9d4-bb69d8eacb34
...
# ╔═╡ 80593660-a960-4ae1-b981-7e31332b023b
rand()
...
begin
	...
	function noisify(x::Number, s)
		result = x + (rand() * rand((1, -1)) * s)
		return myclamp(result)
	end
	
	function noisify(color::AbstractRGB, s)
		r = noisify(red(color), s)
		g = noisify(green(color), s)
		b = noisify(blue(color), s)
		return RGB(r, g, b)
	end
...
philip = let
	...
	float32.(decimate(original, 8))
end

# ╔═╡ c8421509-c839-4ce9-9ff4-453e233a4c0f
typeof(philip)

# ╔═╡ 5706e352-0278-4a53-a264-a42805d43e16
print(philip[1])
...
function blur_1D(v, l)
	...
	len = length(result)
	for i in 1:len
		low = clamp(i-l, 1, length(result))
		hi = clamp(i+l, 1, length(result))
		result[i] = mean(v[low:hi])
	end
	return result
end
...
# ╔═╡ d9f37d84-4608-46ce-9b8e-c0a845de4f4f
colored_line(v)

# ╔═╡ 064bfc56-93f7-4348-846d-22c4d689c723
@bind l_box Slider(1:15, show_value=true)
...
# ╔═╡ 66a8f1ac-0df9-46ed-aa25-b1f34b6301af
md"Note: The above formula is misleading. Sure the $n$ runs between $-l$ and $l$ for the $v_{i-n}$ part, but $n$ just runs regularly along vector $k$. 

I.e. of course k doesn't start at -1 >:("
...
# ╔═╡ d7055372-5f3d-4629-9111-c103d3038c83
begin
	x = [1, 10, 100]
...
@bind gaussian_kernel_size_1D Slider(1:15, show_value=true)
...
function convolve_image(M::AbstractMatrix, K::AbstractMatrix)
	...
	for i in 1:rows, j in 1:cols
		# accumulating addends makes the algorithm works for
		# matrices holding different types.
		#
		# If I don't do this, I need to set M[i, j] to
		# 0 which works fine for Gray but not RGB.
		addends = []
		for m in -l:l, n in -k:k
			mult = extend_mat(M, i-m, j-n) * K[m+l+1, n+k+1]
			push!(addends, mult)
		end
		result[i, j] = sum(addends)
	end
	return result
end
...
# ╔═╡ 26432db9-4b10-44ca-a0ef-8592bd246cee
typeof(philip)
...
# ╔═╡ 25a18c51-f000-456d-87fa-9851017b67f4
md"They should mention here that it's important that when making the kernel, you need to call this function while understanding 0, 0 as the center."

# ╔═╡ e5c56200-aa4f-4848-86a0-efb260b7bfc8
function gaussian_kernel(x, y)
	frac = 1 / (2π)
	exp_frac = -(x^2 + y^2) / 2
	exponent = exp(exp_frac)
...
# ╔═╡ 5964c354-1c0f-4342-ab75-c6d9f85e520b
begin
	test_gaussian_kernel = zeros(3,3)
	rows, cols = size(test_gaussian_kernel)
	l = (rows - 1) ÷ 2
	for i in -l:l, j in -l:l
		test_gaussian_kernel[i + l + 1, j + l + 1] = gaussian_kernel(i,j)
	end
	s = sum(test_gaussian_kernel)
	test_gaussian_kernel = map(x -> x/s, test_gaussian_kernel)
end

# ╔═╡ 4d32634b-61d6-48e4-9549-5d2764d11777
sum(test_gaussian_kernel)

# ╔═╡ aad67fd0-ee15-11ea-00d4-274ec3cda3a3
function with_gaussian_blur(image)
	return convolve_image(image, test_gaussian_kernel)
end
...
# ╔═╡ 618d3858-bf73-47c4-b96f-b5f93e0fb73c
begin
	g_x_mat = [1;2;1] * [1 0 -1]
	g_y_mat = [1;0;-1] * [1 2 1]
end

# ╔═╡ 97ba72fa-7208-48d1-b762-2e2e6b6bd715
g_x = convolve_image(philip, g_x_mat)

# ╔═╡ 5f965d96-2e4d-4e05-8882-dd6f9df88871
g_y = convolve_image(philip, g_y_mat)

# ╔═╡ 7d59f267-7d60-4332-bff4-e3c3bcf78edd
g_sum = map(x -> mapc(*, x, x), g_x) + map(x -> mapc(*, x, x), g_y)

# ╔═╡ af67464f-45a1-48a4-b301-6fa8ca8d39bc
map(x -> mapc(√, x), g_sum)

# ╔═╡ 9eeb876c-ee15-11ea-1794-d3ea79f47b75
function with_sobel_edge_detect(image)
	g_x = convolve_image(image, g_x_mat)
	g_y = convolve_image(image, g_y_mat)
	g_sum = map(x -> mapc(*, x, x), g_x) + map(x -> mapc(*, x, x), g_y)
	g_total = map(x -> mapc(√, x), g_sum)
	return g_total
end
