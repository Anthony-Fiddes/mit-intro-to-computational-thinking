...
function box_blur_kernel(l)
	kernel_len = 2l + 1
	return repeat([1/(kernel_len)], kernel_len)
end
...
function extend(M::AbstractMatrix, i, j)
	rows, cols = size(M)
	return M[clamp(i, 1, rows), clamp(j, 1, cols)]
end
...
function box_blur(v::AbstractArray, l)
	...
	# put sums in each cell
	for i in eachindex(v), m in -l:l
		result[i] += extend(v, i + m)
	end
	# divide each sum to get the mean
	result = result ./ (2l + 1)
	return result
end
...
# ╔═╡ d7af4106-a969-46c8-9669-3f7b91ec39fc
...
# ╔═╡ 5299ad34-f8d0-4a34-99dd-30ccede4f39e
md"""$$M'_{i, j} = \sum_{k, l}  \, M_{i- k, j - l} \, K_{k, l},$$"""

# ╔═╡ 8b96e0bc-ee15-11ea-11cd-cfecea7075a0
function convolve(M::AbstractMatrix, K::AbstractMatrix)
	result = zeros(eltype(M), size(M)...)
	rows, cols = size(result)
	krows, kcols = size(K)
	@assert krows == kcols "Convolve does not work if the kernel is not square"
	offset = (krows - 1) ÷ 2
	for i in 1:rows, j in 1:cols
		# I needed this change for it to work with images. Why?
		sum = zero(eltype(M))
		for k in -offset:offset, l in -offset:offset
			m_component = extend(M, i + k, j + l)
			k_component = K[k + offset + 1, l + offset + 1]
			sum += m_component * k_component
		end
		result[i, j] = sum
	end
	return result
...
convolved_test = convolve(test_image_with_border, K_test)

# ╔═╡ 3099557f-b4c9-449b-a78c-c88e9e8f0a94
typeof(convolved_test)

# ╔═╡ 2b2c7e2b-61f4-4a5a-9f52-fb65448bd73b
eltype(convolved_test)
...
function gaussian_kernel_1D(n; σ = 1)
	kernel_length = 2n + 1
	result = zeros(kernel_length)
	for i in -n:n
		result[i + n + 1] = gauss(i, σ)
	end
	# normalize kernel
...
# ╔═╡ 9f82ea57-41ff-4514-8409-73fe2c219c9b
gaussian_kernel_1D(3)

# ╔═╡ 2141ceda-6d08-4173-97a2-507adfe50d61
sum(gaussian_kernel_1D(3))
...
# ╔═╡ 6f7ee0ff-71e2-4247-ab9c-59623ebcf551
function gaussian_kernel(σ, length)
	result = zeros(Float64, length, length)
	offset = (length - 1) ÷ 2
	for i in -offset:offset, j in -offset:offset
		result[i + offset + 1, j + offset + 1] = gauss(i, j, σ=σ)
	end
	result = result ./ sum(result)
	return result
end

# ╔═╡ 58c74868-1886-4e8e-bf26-74547f756007
gaussian_kernel(3, 5)

# ╔═╡ aad67fd0-ee15-11ea-00d4-274ec3cda3a3
function with_gaussian_blur(image; σ=3, l=5)
	gauss_kern = gaussian_kernel(σ, l)
	return convolve(image, gauss_kern)
end
...
# for some reason this was from 0:20, why?
# is the l not the length of the kernel?
@bind face_l Slider(1:20; show_value=true)

# ╔═╡ 24c75cfb-ead0-4e06-8c7b-71e1e6d9cda8
with_gaussian_blur(philip; σ=face_σ, l=face_l)
...
# ╔═╡ 1782e6d7-ac1d-401b-b8b0-55f450bca858
md"""A low σ value weakens the effect of the blur, and a low face\_l value also weakens the effect of the blur. It appears that σ affects the blur by strengthening the effect of the kernel, and face\_l affects the blur by sampling from more neighboring pixels as the kernel grows larger."""
