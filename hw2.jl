...
student = (name = "Anthony", kerberos_id = "jazz")
...
# ╔═╡ 80348abb-6fb3-43af-a67b-eb7256e70b56
vcat_reduction = 1029-687
...
# ╔═╡ 204b1b99-2b12-4bb5-a95b-7cd911973a90
test_matrix = [1 4 2; 2 5 2; 3 1 3]
...
Note: I actually had to edit the exercises since I came up with the view solution first lol.

This optimization reduced an estimated 342 allocations.

These allocations are explained by the 342 removed calls to vcat that happen while iterating over the rows of the image.
"""
...
function remove_in_each_row_views(img, column_numbers)
	...
	for (i, j) in enumerate(column_numbers)
		...
		img′[i, j:end] = view(img, i, j+1:size(img, 2))
	end
	img′
end
...
This optimization reduced an estimated 684 allocations.

For each of the 342 rows, 1 allocations is made when slicing each side of the row. That's 2 allocations per row, so $342*2=684$
"""
...
# ╔═╡ 7907855b-f325-4a62-90fe-9798455a26dc
"""
    get_neighbors(mat::AbstractMatrix, i::Integer, j::Integer)

Return a view of the 3 neighbors below the pixel at [i, j]

The index 1 indicates the neighbor to the left, while 3 indicates the neighbor to the right.

Note: Index 3 is not guaranteed to exist, but 1 and 2 are
"""
function get_neighbors(mat::AbstractMatrix, i::Integer, j::Integer)
	rows, cols = size(mat)
	@assert i != rows "get_neighbors cannot be called on the last row of a matrix"
	if j == 1
		# special case where we have to pad the output
		# so that its indices can easily be transformed
		# from [1:3] to [-1:1] by subtracting 2
		return vcat([Inf], view(mat, i+1, j:j+1))
	end
	if j == cols
		return view(mat, i+1, j-1:j)
	end
	return view(mat, i+1, j-1:j+1)
end
...
function greedy_seam(energies, starting_pixel::Int)
	...
	seam = zeros(Integer, rows)
	# start = findmin(view(energies, 1, :))[1]
	seam[1] = starting_pixel
	for i in 1:length(seam)-1
		neighbors = get_neighbors(energies, i, seam[i])
		# index goes from 1:3, so transform to -1:1 to
		# adjust from seam[i]
		seam[i+1] = (findmin(neighbors)[2] - 2) + seam[i]
	end
	return seam
end
...
function least_energy(energies, i, j)
	...
	for k in -1:1
		neighbor_col = j + k
		if neighbor_col < 1 || neighbor_col > cols
			continue
		end
		energy = energies[i, j] + first(least_energy(energies, i+1, neighbor_col))
		option = (energy, neighbor_col)
		push!(options, option)
	end
	return options[findmin(first, options)[2]]
end

# ╔═╡ 50c1572c-4f8b-4a25-8f8e-87e7a07cc96c
least_energy(test_matrix, 3, 3)

# ╔═╡ 96c29c61-e5d2-4989-abd9-2d7ac5fc97f6
least_energy(test_matrix, 2, 3)

# ╔═╡ 87d3b3f8-0e01-48b5-b4a8-5645f5093731
least_energy(test_matrix, 1, 3)

# ╔═╡ 85ffffb6-a1c1-4544-b951-0ce9a49ef4df
md"""Test Cases"""

# ╔═╡ a93e3bc2-1831-4ab6-afbb-d36a90adfe3f
# ╠═╡ show_logs = false
begin
	result_mat = copy(test_matrix)
	rows, cols = size(result_mat)
	for i in 1:rows, j in 1:cols
		result_mat[i, j] = least_energy(test_matrix, i, j)[1]
	end
	result_mat
end

# ╔═╡ 9d1cbd17-9f7d-4227-9315-03a0febe008e
begin
	result_mat_2 = copy(test_matrix)
	for i in 1:rows, j in 1:cols
		result_mat_2[i, j] = least_energy(test_matrix, i, j)[2]
	end
	result_mat_2
end
...
This algorithm does an exhaustive search of all possible paths because its base case is at the bottom of the image and the algorithm starts at the top. This is effectively a depth first search.

The number of possible seams grows to $3^m$ for an $m \times n$ image because it roughly triples with each row.
"""
...
function memoized_least_energy(energies, i, j, memory)
	# the insight I gained from looking at someone else's code is that this function can just return the lowest energy, it doesn't have to care about the index. Why did they switch from one function signature to another? I don't know.
	# honestly, I couldn't figure out what was wrong with my memoized functions... sad :(
	# I'm not convinced that it's due to some incompatibility between my code and the framework used here. Anyhow, it's time to move on.
	# base case
	rows, cols = size(energies)
	if i == rows
	   return energies[i, j], j # no need for recursive computation in the base case!
	...
	for k in -1:1
		...
		energy = energies[i, j] + first(memoized_least_energy(energies, i+1, neighbor_col, memory))
		option = (energy, neighbor_col)
		push!(options, option)
	end
	result = options[findmin(first, options)[2]]
	memory[i, j] = result
	return memory[i, j]
end
...
function recursive_memoized_seam(energies, starting_pixel)
	memory = Dict{Tuple{Int,Int}, Tuple{Float64, Int}}() # location => pair
...
function matrix_memoized_least_energy(energies, i, j, memory)
	...
	if i < 1 || j < 1 || i > rows || j > cols
		# I know it's probably not best practice, but it'll work
		return Inf
	end
	if i == rows
	...
	for k in -1:1
		...
		energy = energies[i, j] + matrix_memoized_least_energy(energies, i+1, neighbor_col, memory)
		push!(options, energy)
	end
	memory[i, j] = min(options...) 
	return memory[i, j]
end
...
function matrix_memoized_seam(energies, starting_pixel)
	...
	for i in 1:length(seam)-1
		...
		min_energy = Inf
		for j in -1:1 .+ seam[i]
			option = matrix_memoized_least_energy(energies, i+1, j, memory)
			if option < min_energy
				min_energy = option
				min_neighbor_index = j
			end
		end
		seam[i+1] = min_neighbor_index
	end
	return seam
end
...
function least_energy_matrix(energies)
	...
	rows, cols = size(energies)
	for i in rows-1:-1:1, j in 1:cols
		best = min(get_neighbors(energies, i, j)...)
		result[i, j] = best + energies[i, j]
	end
	return result
end
...
function seam_from_precomputed_least_energy(energies, starting_pixel::Int)
	...
	rows, cols = size(least_energies)
	
	# Replace the following line with your code.
	result = zeros(Integer, rows)
	result[1] = starting_pixel
	for i in 1:rows-1
		last_col = result[i]
		neighbors = get_neighbors(least_energies, i, last_col)
		min_energy = Inf
		min_col_offset = 0
		for (k, energy) in enumerate(neighbors)
			if energy < min_energy
				min_energy = energy
				min_col_offset = k - 2
			end
		end
		result[i+1] = last_col + min_col_offset
	end
	return result
end
